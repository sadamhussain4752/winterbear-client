...
const Home2 = () => {
  ...
  const { data, loading: bannerLoading, error: bannerError } = useSelector((state) => state.data);
  const { wishlist, addloading: addloadingLoading, error: productListErrors } = useSelector((state) => state.wishlist);
  const { productOldlist, loading: productListLoading, error: productListError } = useSelector((state) => state.productOldlist);
  ...
  const loadMoreProducts = (brandId) => {
    setVisibleProducts((prevState) => ({
      ...prevState,
      [brandId]: (prevState[brandId] || 8) + 4, // Increase the number of visible products by 4
    }));
  };
          ...
          <div className="row">
            {data && data.Categorys && data.Categorys.map((item) => (
              <div className="col-md-3 col-6 mt-2" key={item._id}>
                <div className="collection-box-1 d-block mx-auto" onClick={() => navigate(`/shop/${item._id}`)}>
                  <div className="image-container-1">
                    <img src={`${item.imageUrl}`} className="mb-0" alt={item.name} />
                    <h4>{item.name}</h4>
                  </div>
                </div>
              </div>
      ...
      <section className="py-5 shop">
        <div className="container">
          ...
          <div className="row justify-content-center">
            {productOldlist && productOldlist.productList && productOldlist.productList.map((item, index) => {
                ...
                <div key={`stickyBody${index}`} className="row">
                  <div className="col-md-2 g-0 bg-transparent sticky d-none">
                    <div className="sticky-logo-2">
                      <img src={`${item.brand.imageUrl}`} className="w-75 d-block mx-auto" alt={item.brand.name} />
                    </div>
                  </div>
                  <div className="mb-3" id="sticky2">
                    <div className="sticky-logo-2" onClick={() => navigate(`/brand/${item.brand._id}`)}>
                      <img src={`${item.brand.imageUrl}`} className="d-block mx-auto sticky-log2" alt={item.brand.name} />
                    </div>
                  </div>
                  <div className="container-fluid shop-by tile-1 px-lg-4 px-0" id={`sticky1${index}`}>
                    <div className="container-fluid">
                      <ul className="nav nav-pills nav-fill shop-by-tab py-3 mb-4" id="pills-tab" role="tablist">
                        {item.subbrand.map((itemname) => (
                          <li className="nav-item py-0" role="presentation" key={itemname._id} onClick={() => navigate(`/brand/${item.brand._id}`)}>
                            <img src={`${itemname.imageUrl}`} className="shopby-top-icoimg" alt={itemname.name} />
                            <button className="nav-link fw-semibold py-0 position-relative rounded-pill" id="pills-koya-tab" data-bs-toggle="pill" data-bs-target="#pills-koya" type="button" role="tab" aria-controls="pills-koya" aria-selected="true">
                              {itemname.name}
                            </button>
                          </li>
                        ))}
                    ...
                    {!visibleProducts[item.brand._id]  ? (
                      <div className="text-center view-more" style={{ cursor: "pointer" }} onClick={() => loadMoreProducts(item.brand._id)}>
                        View more <br />^
                      </div>
                    ):(<div className="text-center view-more" style={{ cursor: "pointer" }} onClick={() => loadallProducts(item.brand._id)}>
                    View All <br />
                  </div>) }
                  </div>
                </div>
